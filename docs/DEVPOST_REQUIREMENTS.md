...
| C10 | [R §4](https://datahub.devpost.com/rules)                                                              | Submitted repo uses Apache 2.0 visible at top/About.                                                                                        | Project text and root metadata remain MIT; relicensing authority, legal compatibility/obligation review of the 138-node declared-license evidence, and coordinated migration are unresolved.                                      | OPEN         | Legal/owner gate          | Resolve the readiness-packet blockers, obtain an independent explicit relicense authorization, then implement and verify GitHub detection separately.                |
...
2. **Licence:** offline frozen-graph evidence records declared licenses for all 138 production nodes,
   legal files for 137, and 0 package NOTICE files. Compatibility, attribution/NOTICE obligations,
   the missing `abstract-logging@2.0.1` legal file, and project-owner/contributor relicensing authority
   remain unresolved. Resolve them before a separate explicit Apache-2.0 authorization; do not
   silently relabel existing third-party or contributor work.
