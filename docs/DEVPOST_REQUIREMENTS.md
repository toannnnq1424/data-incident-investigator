...
| C09 | [R §4](https://datahub.devpost.com/rules)                                                              | Public source repository with all necessary source/assets/instructions.                                                                     | GitHub remains Private. The bounded tree/history audit found no credible credential/private key, but branch, commit, issue, PR, Actions, tag, and Draft Release metadata still require owner review.                              | OPEN         | Separate publication gate | Resolve the residual review items in the [readiness packet](PUBLIC_SOURCE_APACHE_READINESS.md), then obtain an independent explicit Public-visibility authorization. |
| C10 | [R §4](https://datahub.devpost.com/rules)                                                              | Submitted repo uses Apache 2.0 visible at top/About.                                                                                        | Project text and root metadata remain MIT; relicensing authority, the 138-node dependency licence/NOTICE inventory, and coordinated migration are unresolved.                                                                     | OPEN         | Legal/owner gate          | Resolve the readiness-packet blockers, obtain an independent explicit relicense authorization, then implement and verify GitHub detection separately.                |
| C11 | [R §§4, 7](https://datahub.devpost.com/rules), [S](https://datahub.devpost.com/resources)              | All third-party SDK/API/data/content use is authorized and licence-compliant.                                                               | The exact production closure has 138 external package-version nodes, including exact-pinned `@modelcontextprotocol/sdk@1.29.0`; allowed local evidence has no package licence fields, so compatibility/NOTICE remains unresolved. | PARTIAL      | Legal/owner + Phase 8.3   | Resolve authoritative licence/notice evidence for every production node, then audit remaining DataHub tool, dataset, image, music, font, and video rights.           |
...
2. **Licence:** all 138 external production package-version licence/NOTICE records remain unknown
   under the allowed local evidence, and project-owner/contributor relicensing authority is not
   attested. Resolve both before a separate explicit Apache-2.0 authorization; do not silently relabel
   existing third-party or contributor work.
...
live/judge credentials. Phase 8.3 prepares the two independent decisions but grants neither. These
phases perform no registration, submission, consent, upload, deployment, credential entry, live
provisioning, licence/visibility change, or tag/Release mutation.
