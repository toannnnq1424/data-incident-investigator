...
**Project submitted!**, **Submitted**, and **5/5**. Remaining boundaries are a durable authorized
remote MCP/judge path, independent repository proof of entrant eligibility/IP/media-rights facts,
organizer acceptance, and future winner verification if applicable. No post-deadline substantive edit
is allowed, so preserve the submitted content and limit any correction to the Rules' permitted scope.
...
judge endpoint or judge authentication path; public/judge live MCP access remains `PARTIAL`.
...
| C18 | [R §4](https://datahub.devpost.com/rules)                                                              | Public functioning-project video is required; the Rules say it should be under three minutes and judges need not watch beyond minute three. | The selected authentic continuous 2:50.20, 1440 × 900 male-voice fixture flow is Public at <https://youtu.be/D5mvMqrhyDc>. It shows the functioning project, retains the verified real video stream, has published authored English captions, and is embedded in the submitted project.                                                                      | PASS                                     | Phase 8.9A–8.11 complete   | Preserve Public playback and authored captions through judging.                                                                                               |
| C19 | [R §4](https://datahub.devpost.com/rules)                                                              | Video publicly visible on YouTube, Vimeo, or Youku.                                                                                         | The selected male-voice video is Public on YouTube at <https://youtu.be/D5mvMqrhyDc>, with authored English captions and no reported copyright-check issue. The exact URL is included in submitted Devpost project `1117401`.                                                                                                                                | PASS                                     | Phase 8.11 complete        | Preserve the exact URL and Public playback through judging; do not confuse submission with organizer acceptance.                                              |
| C20 | [R §4](https://datahub.devpost.com/rules)                                                              | Video avoids unauthorized trademarks/music/material and is English or translated.                                                           | The Public video uses only the project UI, synthetic fixture, offline Microsoft Mark narration, and authored English captions; it has no music or added third-party visual media, YouTube reported no copyright-check issue, and its URL is linked. Repository evidence still does not provide a durable entrant rights/IP attestation.                      | PARTIAL                                  | Phase 8.9A–8.11 / entrant  | Retain truthful rights/privacy/accessibility evidence and respond to any organizer verification request.                                                      |
...
| C25 | [O](https://datahub.devpost.com/), [R §4](https://datahub.devpost.com/rules)                           | Screenshots/images required by the form.                                                                                                    | Five clean 1440 × 900 PNGs from the one canonical synthetic fixture incident are submitted with aligned English captions, and the completed report/export image is the thumbnail.                                                                                                                                                                            | PASS                                     | Phase 8.8–8.11 complete    | Preserve the exact authentic image/caption mapping; do not add another incident or synthetic UI.                                                              |
...
| C28 | [R §6](https://datahub.devpost.com/rules)                                                              | Stage 1 theme and required API/SDK viability.                                                                                               | Named DataHub MCP integration is implemented, product-tested, and validated against localhost DataHub Core `1.6.0` plus official MCP `0.6.0`; the project is submitted, but a durable remote judge MCP service remains unavailable.                                                                                                                          | PARTIAL                                  | Phase 8.2–8.11             | Preserve the local OSS proof without presenting it as public live evidence; add remote judge access only if separately authorized.                            |
| C29 | [R §6](https://datahub.devpost.com/rules)                                                              | Stage 2 equal criteria: DataHub, execution, originality, usefulness, submission quality.                                                    | The submitted project combines the claim-mapped English story, Public app/repository, embedded authentic YouTube take, and five captioned fixture images. Real local DataHub OSS/MCP evidence now exists; live DataHub inputs can vary and no durable remote judge MCP endpoint is available.                                                                | PARTIAL                                  | Phase 8.2–8.11             | Preserve the truthful matrix mapping and do not claim live-mode determinism, organizer acceptance, or scoring.                                                |
...
Current matrix totals after the Phase 8.11 operating-window, registration, hosted-video, real local
OSS/MCP validation, and submitted 5/5 project evidence are
**15 PASS / 1 QUALIFIED PASS — OWNER-AUTHORIZED SCOPE / 8 PARTIAL / 3 OPEN / 10 NOT REQUIRED**
across 37 rows.

## Open blockers and verification questions

1. **Remote judge MCP evidence:** protocol/product tests and a real localhost-only DataHub Core
   `1.6.0` + official MCP `0.6.0` proof pass, including readiness, search, bounded lineage, explicit
   unsupported recent changes, and one safe degraded incident. No authorized durable remote
   DataHub Core/Cloud MCP endpoint or judge authentication path has been provisioned. Do not present
   the local OSS proof as public live evidence.
...
6. **Video host/link:** the selected authentic 2:50.20 male-voice derivative is Public at
   ...
   `PASS`; no organizer acceptance or prize result is claimed.
...
Phase 8.11 advances the technical named-integration seam to **PASS — LOCAL OSS** while remote
public/judge MCP access remains `PARTIAL`. Integrated Phase 8.4A makes C09 `PASS`; verified Phase 8.4B
...
links, truthful English content, and judge media/information. No organizer acceptance, prize result,
durable entrant eligibility/IP attestation, remote MCP provisioning, or tag/Release mutation is
claimed.
