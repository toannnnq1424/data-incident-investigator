...
The current repository is MIT-licensed in `LICENSE`, root `package.json`, and the README. Therefore the
submission's Apache 2.0 condition is **OPEN**, not satisfied by the RC. A later legal/licensing decision
must determine a valid transition or compatible arrangement; Phase 8.1 makes no licence change.
...
These are repository facts at starting `main`
`c4e33f7af3707f604d35b1220a18e4e83f491be3` (tree
`ffa4276315f8dd788f12b2780cee9bc13365ebbc`), not competition claims:
...
- The supported target is a generic Node 24 host serving static web output with a same-origin `/api`
  ...
  [deployment](DEPLOYMENT.md).
- The current DataHub implementation is a bounded direct GraphQL adapter. No current source implements
  DataHub MCP Server, Agent Context Kit, DataHub Skills, or Analytics Agent, so the named integration
  rule is open.
...
| C04 | [R §4](https://datahub.devpost.com/rules)                                                              | Project is newly created during the Submission Period.                                         | First repository commit is 2026-07-18, after the 2026-07-06 opening.                                                                             | PASS         | Phase 8.1                 | Preserve Git history; do not rewrite it.                                                                                   |
...
| C06 | [R §4](https://datahub.devpost.com/rules)                                                              | Working application uses open-source DataHub plus at least one named integration.              | Direct GraphQL adapter exists; none of MCP Server, ACK, Skills, or Analytics Agent is implemented.                                               | OPEN         | Phase 8.2                 | Implement and verify one named integration; show it in runtime evidence, video, and copy.                                  |
| C07 | [R §4](https://datahub.devpost.com/rules)                                                              | App installs/runs consistently on its stated intended platform.                                | Node 24 release artifact/runbook and fixture smoke evidence exist; qualifying competition integration and judge path do not.                     | PARTIAL      | Phase 8.2                 | Validate the final named-integration path from clean judge instructions on the declared platform.                          |
...
| C09 | [R §4](https://datahub.devpost.com/rules)                                                              | Public source repository with all necessary source/assets/instructions.                        | GitHub repository is private; source and setup docs exist.                                                                                       | OPEN         | Separate publication gate | After secret/history/readiness review and separate authorization, make the exact submission repo public.                   |
| C10 | [R §4](https://datahub.devpost.com/rules)                                                              | Submitted repo uses Apache 2.0 visible at top/About.                                           | Current repo is MIT-licensed.                                                                                                                    | OPEN         | Legal/owner + Phase 8.2   | Decide and implement a legally valid Apache 2.0 licensing state; verify GitHub detection before submission.                |
| C11 | [R §§4, 7](https://datahub.devpost.com/rules), [S](https://datahub.devpost.com/resources)              | All third-party SDK/API/data/content use is authorized and licence-compliant.                  | Lockfile/licence metadata and synthetic fixtures exist; no final submission asset/data audit exists.                                             | PARTIAL      | Phase 8.2–8.3             | Audit dependency, DataHub tool, dataset, image, music, font, and video rights; record attribution/disclosures.             |
...
| C18 | [R §4](https://datahub.devpost.com/rules)                                                              | Required functioning-project video, under three judged minutes.                                | Demo script exists; no final video.                                                                                                              | OPEN         | Phase 8.3                 | Record final functioning named-integration flow, keep it below 3:00, and verify every claim.                               |
...
| C28 | [R §6](https://datahub.devpost.com/rules)                                                              | Stage 1 theme and required API/SDK viability.                                                  | Theme fit is credible, but named DataHub integration is missing.                                                                                 | OPEN         | Phase 8.2                 | Make the named integration visible and verifiable before polishing secondary criteria.                                     |
| C29 | [R §6](https://datahub.devpost.com/rules)                                                              | Stage 2 equal criteria: DataHub, execution, originality, usefulness, submission quality.       | Strong deterministic execution/evidence story exists; no final DataHub integration or submission package.                                        | PARTIAL      | Phase 8.2–8.3             | Map final evidence to all five criteria; do not convert equal weighting into an organizer-published percentage claim.      |
...
| C33 | [O](https://datahub.devpost.com/), [R §§4, 6](https://datahub.devpost.com/rules)                       | LLM/model call.                                                                                | Current RC is deterministic and makes zero model calls; official pages do not mandate one.                                                       | NOT REQUIRED | —                         | Preserve the truthful no-model statement unless a separately scoped implementation changes it.                             |
...
1. **Named DataHub integration:** literal Rules language requires at least one of MCP Server, Agent
   Context Kit, DataHub Skills, or Analytics Agent. Current direct GraphQL does not prove compliance.
2. **Licence:** resolve Apache 2.0 requirements against the current MIT repository with an owner/legal
   decision; do not silently relabel existing third-party or contributor work.
3. **Public repository:** required for submission, but visibility remains private until a separately
   authorized gate passes secret/history/readiness review.
...
Phase 8.1 resolves none of these by external mutation. It performs no registration, submission,
consent, upload, deployment, credential entry, licence/visibility change, tag/Release change, or
Phase 8.2 product implementation.
