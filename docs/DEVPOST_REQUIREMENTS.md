...
| C11 | [R §§4, 7](https://datahub.devpost.com/rules), [S](https://datahub.devpost.com/resources)              | All third-party SDK/API/data/content use is authorized and licence-compliant.                                                               | Cloud Run distributes its runtime dependencies. The correction binds lock SHA `eeec795d…84b2`, 152 production identities, 149 runtime identities/roots, five Vite identities, deterministic notices, Apache legal files, and the truthfully labeled `abstract-logging@2.0.1` fallback. Local checks pass; the live image predates this evidence. | BLOCKED — CLOUD RUN DEPLOYMENT EVIDENCE | Phase 8.7 correction    | Deploy the clean immutable commit and verify its live digest/runtime closure, legal files, notices, and source binding. No blanket legal clearance is claimed. |
...
**3 PASS / 11 PARTIAL / 14 OPEN / 1 BLOCKED / 8 NOT REQUIRED**
across 37 rows.
...
2. **Licence:** integrated exact main satisfies C09. The historical Phase 8.5 release graph has 138
   ...
   `abstract-logging@2.0.1`; source and local production image verification pass. C11 is still
   `BLOCKED` because the live image predates that evidence. No blanket legal clearance is claimed;
   C12 ownership/asset attestations and final submission review remain separate.
