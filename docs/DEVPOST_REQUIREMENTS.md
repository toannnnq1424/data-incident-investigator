...
captions/transcript, Opus audio, and no new incident or UI frame. Both are Public GitHub repository
assets. Neither has been uploaded to or made publicly visible on YouTube, Vimeo, or Youku, linked or
entered in Devpost, accepted by an organizer, or submitted. GitHub blob/raw access does not satisfy
the Rules' video-host/link requirement. C18 remains only `PARTIAL — LOCAL CANDIDATE`; C19 remains
`OPEN`.
...
| C08 | [R §4](https://datahub.devpost.com/rules)                                                              | Submission functionality matches text and video.                                                                                            | The draft and claim matrix map each product claim to the real fixture UI/repository evidence. The authentic Phase 8.9 flow now has silent and exact-frame synchronized male-voice repository variants with matching English captions; no Rules-compliant YouTube/Vimeo/Youku video is linked in Devpost and no form is submitted.                            | PARTIAL                                 | Phase 8.8–8.9A / entrant | Recheck the entrant-approved Rules-hosted export and every final form field against the functioning take.                                                |
| C09 | [R §4](https://datahub.devpost.com/rules)                                                              | Submitted repo uses Apache 2.0 visible at top/About.                                                                                        | Exact current main `b4f9e360e22baba3cab60710a65ea5c0e0555369` preserves canonical Apache-2.0 licensing; exact-main CI run `30329118539` / job `90180425391` is successful and GitHub detects `Apache-2.0`.                                                                                                                                                   | PASS                                    | Phase 8.4A complete      | Preserve exact license/main evidence through later work and recheck detection before submission.                                                         |
| C10 | [R §4](https://datahub.devpost.com/rules)                                                              | Public source repository with all necessary source/assets/instructions.                                                                     | GitHub is Public at exact current main `b4f9e360e22baba3cab60710a65ea5c0e0555369`, tree `29f51960e9a772b8c331b427943920f296031bbe`, which integrates Phase 8.9A. The running Cloud Run image remains separately bound to immutable source `3653cf6b…7fa4f`; later docs/media merges were not redeployed.                                                     | PASS                                    | Phase 8.4B complete      | Preserve and reverify Public access, reporting, exact-main evidence, and the separately governed external service through later work.                    |
...
| C17 | [R §4](https://datahub.devpost.com/rules)                                                              | State intended platform and complete setup/testing instructions.                                                                            | The concise public fixture path plus Node 24 / pnpm `11.9.0` repository-local fallback are rehearsed; unrestricted access through the full judging period remains unresolved under C14.                                                                                                                                                                      | PARTIAL                                 | Phase 8.8–8.9 / operator | Resolve the judging-access window and recheck the exact fallback immediately before submission.                                                          |
| C18 | [R §4](https://datahub.devpost.com/rules)                                                              | Public functioning-project video is required; the Rules say it should be under three minutes and judges need not watch beyond minute three. | One authentic continuous 2:50.20, 1440 × 900 VP8/WebM app-only fixture take exists as an unchanged silent source and a video-stream-identical Opus derivative with synchronized male English narration/captions. They are GitHub repository assets, not YouTube/Vimeo/Youku-hosted or Devpost-linked final submission videos.                                | PARTIAL — LOCAL CANDIDATE               | Phase 8.9A / entrant     | Entrant reviews the chosen export/rights and performs the Rules-listed host upload and Devpost linking only under separate authorization.                |
| C19 | [R §4](https://datahub.devpost.com/rules)                                                              | Video publicly visible on YouTube, Vimeo, or Youku.                                                                                         | GitHub blob/raw access exists, but there is no YouTube/Vimeo/Youku URL or hosting/visibility selection and no video link entered in Devpost. GitHub repository access does not satisfy this requirement.                                                                                                                                                     | OPEN                                    | Phase 8.3                | Upload only under separate authorization; use Public visibility unless organizer confirms unlisted, then link the approved URL in Devpost.               |
| C20 | [R §4](https://datahub.devpost.com/rules)                                                              | Video avoids unauthorized trademarks/music/material and is English or translated.                                                           | Both variants use only the project UI and synthetic fixture with no music or added third-party visual media. The voiced derivative adds only offline Microsoft Mark synthetic English speech plus exact captions/transcript; frame, audio, privacy, and media-rights checks pass, while entrant review of any Rules-hosted export remains open.              | PARTIAL                                 | Phase 8.9A / entrant     | Entrant performs the final rights/privacy/accessibility review and retains evidence before any authorized YouTube/Vimeo/Youku upload.                    |
| C21 | [R §4](https://datahub.devpost.com/rules)                                                              | All submitted materials are English or English-translated.                                                                                  | The judge-facing draft, quickstart, matrix, script, silent captions/transcript, and synthetic male narration with matching captions/transcript are English; the final form and a Rules-listed hosted-and-linked video remain pending.                                                                                                                        | PARTIAL                                 | Phase 8.8–8.9A / entrant | Run a final English/translation completeness review over every submitted field and hosted asset.                                                         |
...
Current matrix totals during Phase 8.10 release preparation are
**3 PASS / 1 QUALIFIED PASS — OWNER-AUTHORIZED SCOPE / 13 PARTIAL / 12 OPEN / 8 NOT REQUIRED**
across 37 rows.
...
2. **Licence:** integrated exact main satisfies C09. The historical Phase 8.5 release graph has 138
   ...
   production audit and source attribution are verified locally; exact clean-commit artifact and
   fresh-checkout gates remain pending until the sole commit. It is not deployed. C11 is
...
3. **Public repository:** C10 is `PASS`. The repository is Public, exact main is
   ...
   `29f51960e9a772b8c331b427943920f296031bbe`, and Phase 8.9A is integrated. The running service
...
6. **Video visibility:** the authentic 2:50.20 source is public on merged `main`, and Phase 8.9A adds
   a synchronized male-voice derivative with the exact same video packets on merged `main` in the
   Public GitHub repository. Neither has been uploaded to or made publicly visible on YouTube, Vimeo,
   or Youku or linked/entered in Devpost. GitHub blob/raw access does not satisfy the official video-
   host/link requirement. C18 remains only `PARTIAL — LOCAL CANDIDATE`; C19 stays `OPEN`. Ask
   `support@devpost.com` whether unlisted hosting qualifies if Public is not feasible; current Rules
   only say publicly visible.
...
functioning-project source and synchronized English review packet. Phase 8.9A adds only an exact-frame
male-voice derivative using an offline installed Windows voice. This does not advance C18/C20 beyond
`PARTIAL`. The assets are publicly accessible through the Public GitHub repository on merged `main`, but
they have not been uploaded to or made publicly visible on YouTube, Vimeo, or Youku, linked or entered
in Devpost, accepted, or submitted. No registration, consent, form save, deployment, credential
entry, live provisioning, or tag/Release mutation occurs.
