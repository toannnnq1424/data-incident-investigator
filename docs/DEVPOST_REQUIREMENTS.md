...
**Deadline risk:** submissions were open at access time. Apache-2.0 is now integrated and detected on
exact main, but the repository still has multiple submission blockers: live/judge evidence for the
named DataHub integration, Public visibility, testable Project URL/access, video, form completion,
entrant eligibility/registration attestations, and submission-specific IP/data review. No
post-deadline substantive edit is allowed, so these must not be left for the final hours.
...
the explicitly bounded zero-cost/data/API scope and are not blanket legal clearance; **PARTIAL** = useful
...
| C09 | [R §4](https://datahub.devpost.com/rules)                                                              | Submitted repo uses Apache 2.0 visible at top/About.                                                                                        | Exact current main `c7abc652c23b532e90091b377490b27eadd7e084` preserves canonical Apache-2.0 licensing; historical exact-main CI run `30188091600` / job `89756253516` is successful and GitHub detects `Apache-2.0`.                                                                                                                            | PASS                                    | Phase 8.4A complete     | Preserve exact license/main evidence through later work and recheck detection before submission.                                                               |
| C10 | [R §4](https://datahub.devpost.com/rules)                                                              | Public source repository with all necessary source/assets/instructions.                                                                     | GitHub is Public at exact current main `c7abc652c23b532e90091b377490b27eadd7e084`; authenticated verification preserves enabled private reporting, Draft RC state, and no deployment environment.                                                                                                                                                | PASS                                    | Phase 8.4B complete     | Preserve and reverify Public access, reporting, and exact-main evidence through later work.                                                                    |
| C11 | [R §§4, 7](https://datahub.devpost.com/rules), [S](https://datahub.devpost.com/resources)              | All third-party SDK/API/data/content use is authorized and licence-compliant.                                                               | Phase 8.5 technical attribution passes for the exact five MIT-declared embedded packages and enforced legal text. The owner authorizes only the exact verified artifact, synthetic/authorized data, and authorized zero-cost DataHub/API scope; `abstract-logging@2.0.1` is not embedded and its missing legal file remains a documented caveat. | QUALIFIED PASS — OWNER-AUTHORIZED SCOPE | Owner + Phase 8.6       | Enforce the no-fee/no-sensitive-data/authorized-instance boundary; retain C12 and final submission rights attestations. No blanket legal clearance is claimed. |
...
Current matrix totals after the Phase 8.7 deployment are
...
2. **Licence:** integrated exact main satisfies C09. The exact frozen graph remains 138 declared
   licenses, 137 legal files, and 0 package NOTICE files. No generic project NOTICE is added. Phase
   8.5 attributes the exact positive rendered web contributions to
   ...
   legal/source hashes, content, and bundle membership. `abstract-logging@2.0.1` is not embedded
   because server imports remain external and `node_modules` is excluded, but its missing legal file
   remains a broader C11 caveat. Phase 8.6 records C11 as
   `QUALIFIED PASS — OWNER-AUTHORIZED SCOPE`: the technical attribution passes and the owner
   authorizes only the exact verified artifact with synthetic/authorized data and authorized
...
3. **Public repository:** C10 is `PASS`. The repository is Public, exact main is
   `c7abc652c23b532e90091b377490b27eadd7e084`, tree
   `66e90eae74c7065c62a30a14ffeb25ef26974ea4`, and authenticated source/collaboration surfaces
...
4. **Judge access:** public credential-free fixture service
   ...
   yet guaranteed; preserve the Public-repository quickstart as fallback and resolve C14 before
   submission.
