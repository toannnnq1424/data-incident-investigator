...
| Card-required       | Koyeb Starter / Free Instance       | Starter requires a valid payment method. One Free Instance is 0.1 vCPU, 512 MB RAM, 2 GB SSD, one region, no Volume, and scales to zero after one idle hour; Koyeb says the free web Service itself is never charged. Paid usage starts when a paid Instance reaches `Starting`; billing is USD plus applicable tax. Billing alerts have a $5 minimum and Koyeb says spending limits are not yet available.                                                                                                                                                             | Node/full-stack shape is plausible, but a payment method unlocks usage-based paid instances and there is no hard zero-dollar cap. Exact verification hold, included outbound/build quota, deletion retention, and a zero-spend cancellation safeguard are not established.                                                                                                                               | **REJECT.** Missing hard zero-dollar control and unresolved hold/quota/retention facts fail the owner policy.                                                                                                                                                              |
...
- Koyeb Starter/Free Instance: signup `https://app.koyeb.com/auth/signup`; usage/billing is the
  ...
  taxes apply. No official hold amount or hard USD 0 cap is documented. **REJECT**; no deadline or
  signup action is authorized.
...
- [Koyeb Instances](https://www.koyeb.com/docs/reference/instances),
  ...
  valid-payment-method requirement, lifecycle/billing facts, USD/tax handling, $5 minimum alert, and
  absence of a spending-limit feature.
